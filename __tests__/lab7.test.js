describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartNumberText = await cartNumber.getProperty('innerText');
    const cartNumberParsed = await cartNumberText.jsonValue();
    expect(cartNumberParsed).toBe('20'); 
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(const item of productItem){
      const shadowRoot = await item.getProperty('shadowRoot');
      const button = await shadowRoot.$('button');
      const buttonTxt = await button.getProperty('innerText');
      const buttonTxtParsed = await buttonTxt.jsonValue();
      expect(buttonTxtParsed).toBe("Remove from Cart"); 
    }
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const cartNumberText = await cartNumber.getProperty('innerText');
    const cartNumberParsed = await cartNumberText.jsonValue();
    expect(cartNumberParsed).toBe('0'); 
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(const item of productItem){
      ...
      const buttonTxt = await button.getProperty('innerText');
      const buttonTxtParsed = await buttonTxt.jsonValue();
      expect(buttonTxtParsed).toBe("Add to Cart"); 
    }
